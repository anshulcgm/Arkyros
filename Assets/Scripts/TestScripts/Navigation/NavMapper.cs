...
public class NavMapper : MonoBehaviour
{
    ...
    public void Update()
    {
        ...
        { 
            ...
            createMap();
            Debug.Log("made maMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMp");
            compareMS = millisecondsBetweenPathfindingCalls;
    ...
    public List<Node> floodFill(Vector3 start, int numTimesToFlood)
    {
        ...
        floodFillRec(startNode, numTimesToFlood, solutions);
        Debug.Log(solutions.Count + " num solns");
        /*
        foreach(Node n in solutions)
        {
            foreach (Node conn in n.connectedNodes)
            {
                if (conn != null)
                {
                    Debug.DrawLine(n.location, conn.location, Color.red, 6000000.0f);
                }
            }
        }
        */
    ...
    //the recursive flood fill function. Don't call this one, use the one above.
    private void floodFillRec(Node start, int numTimesToFlood, List<Node> alreadyAddedNodes)
    {        
        //end for recursion and check for if start is null
        if (numTimesToFlood <= 0 || start == null)
        { 
            return;
        }
        start.searched = false;

        //add start if start is not added
        if (!alreadyAddedNodes.Contains(start))
        {
            alreadyAddedNodes.Add(start);
        }
        ...
        foreach (Node n in start.connectedNodes)
        {
            ...
            n.searched = false;
            if (!alreadyAddedNodes.Contains(n))
            {
                alreadyAddedNodes.Add(n);
                //call the recursion on the new node
                floodFillRec(n, numTimesToFlood - 1, alreadyAddedNodes);
            }
        }
    }
    ...
    //gets the sphere of nodes around a point.
