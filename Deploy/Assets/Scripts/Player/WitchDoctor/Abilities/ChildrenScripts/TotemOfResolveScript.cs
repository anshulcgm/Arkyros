...
public class TotemOfResolveScript : MonoBehaviour
{
    DateTime start;
        ...
        foreach (GameObject player in array)
        {
            if (Vector3.Distance(player.transform.position, transform.position) <= radius) //if the player is within 20 units of the totem
            {
                if (!affectedPlayers.Contains(player))//if its not already in the List, add it in and give it the buff
                {
                    ...
                    player.GetComponent<Stats>().allStats[(int)stat.Defense, (int)statModifier.Multiplier] *= multiplier;
    ...
    public void augment()
    {
        ...
        foreach (GameObject player in affectedPlayers)
        {
            player.GetComponent<Stats>().allStats[(int)stat.Defense, (int)statModifier.Multiplier] *= 4f;
            player.GetComponent<Stats>().allStats[(int)stat.Defense, (int)statModifier.Multiplier] /= 3f;
        }
    }
}
