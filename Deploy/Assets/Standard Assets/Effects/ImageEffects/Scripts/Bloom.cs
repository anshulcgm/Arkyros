...
namespace UnityStandardAssets.ImageEffects
{
    ...
    public class Bloom : PostEffectsBase
    {
        ...
        public float maxObscureEffect;
        public float thresholdAtmosphereColorChangeToUpdate = 0.1f;

        //call base start (so things don't get messed up) and then set the camera's depthTextureMode.
        public new void Start()
        {
            base.Start();
            GetComponent<Camera>().depthTextureMode = DepthTextureMode.Depth;
            maxObscureEffect = 1;
            depthShaderMat = (Material)Resources.Load("distMat");
        }
        ...
        private void AtmosphericObscuring(float distBegin, float distEffectMaximized, float maxObscureEffect, RenderTexture from, RenderTexture to)
        {
            
            if (depthShaderMat == null)
            {
                return;
            }

            RenderTexture depthShadedRenTex = new RenderTexture(from);
            Graphics.Blit(from, depthShadedRenTex, depthShaderMat);

            //get depth-based shader in Tex2D
            Texture2D depthShadedTex2D = new Texture2D(depthShadedRenTex.width, depthShadedRenTex.height, TextureFormat.RGB24, false);
            RenderTexture.active = depthShadedRenTex;
            depthShadedTex2D.ReadPixels(new Rect(0, 0, depthShadedTex2D.width, depthShadedTex2D.height), 0, 0);

            //possibly need an 'apply' here...

            RenderTexture.active = null;

            Texture2D cameraScreenShot = new Texture2D(from.width, from.height, TextureFormat.RGB24, false);
            RenderTexture.active = from;
            cameraScreenShot.ReadPixels(new Rect(0, 0, from.width, from.height), 0, 0);
            


            Camera cam = GetComponent<Camera>();
            float skyIntensity = GetIntensityNoAlpha(cam.backgroundColor);
            Color32[] depthPixelColors = depthShadedTex2D.GetPixels32();
            Color32[] cameraPixelColors = cameraScreenShot.GetPixels32();

            int index = 0;
            for(int i = 0; i < depthPixelColors.Length; i++) {
                Color32 pixelColor = cameraPixelColors[i];
                float pixelIntensity = GetIntensityNoAlpha(pixelColor);
                float pixelDepth = depthPixelColors[i].r;

                if(pixelIntensity < skyIntensity && pixelDepth > distBegin)
                {
                    cameraPixelColors[i] = cam.backgroundColor;                       
                }
                index++;                
            }

            cameraScreenShot.SetPixels32(cameraPixelColors);
        ...
        //accepted method of getting intensity of color for human eyes.
        //link to formula docs here: [https://en.wikipedia.org/wiki/Luma_(video)]
        //this does not take transparancy into account.
        float GetIntensityNoAlpha(Color32 c)
        {
           return (float)(0.2126 * c.r + 0.7152 * c.b + 0.0722 * c.g);
        }
